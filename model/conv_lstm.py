...
class ConvLSTM(Model):
    ...
    def make_network(self, input_opr, name, train=True, reuse=False, batch_size=0):
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            value_out = self.value_output_layer(cell_out, train, w_reg)

            if self.is_continuous:
                policy_out = self.continuous_policy_output_layer(cell_out, train, w_reg)
            else:
                policy_out = self.discrete_policy_output_layer(cell_out, train, w_reg)
